...
namespace NetworKit {
    ...
    long ONPL::perf_event_open(struct perf_event_attr *hw_event, pid_t pid, int cpu, int group_fd, unsigned long flags){
        int ret;
...
//                timing["refine"].push_back(timer.elapsedMilliseconds());
    ...
    std::map<std::string, std::vector<long long > > ONPL::getCacheCount() {
        return cache_info;
    }

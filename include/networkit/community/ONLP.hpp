...
#ifndef NETWORKIT_COMMUNITY_ONLP_HPP_
#define NETWORKIT_COMMUNITY_ONLP_HPP_
 ...
 *
 */
class ONLP final : public CommunityDetectionAlgorithm {
...
public:
     ...
     */
    ONLP(const Graph& G, count theta = none, count maxIterations=none);
     ...
     * @param[in] theta updateThreshold: number of nodes that have to be changed in each iteration so that a new iteration starts.
     */
    ONLP(const Graph &G, const Partition &baseClustering, count theta = none);
...
#endif // NETWORKIT_COMMUNITY_ONLP_HPP_
